...
for repeat in range(repeats):
    ...
    # Plan actions for each agent for the whole day
    for agent in agents:
        agent.plan(8, prompt_meta)    # global_time => 8, wake up in 8
        if log_plans:
            log_output += f"{agent.name} plans: {agent.plans}\n"
            if print_plans:
                print(f"{agent.name} plans: {agent.plans}")

    # Execute action and reaction for each agent
    for time_hour in range(8, 25):
        global_time = time_hour
